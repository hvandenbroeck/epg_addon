...
def _get_solar_charge_config() -> dict:
    ...
    return {
        'production_l1': opts.get('ev_solar_charge_production_phase_l1_entity'),
        'production_l2': opts.get('ev_solar_charge_production_phase_l2_entity'),
        'production_l3': opts.get('ev_solar_charge_production_phase_l3_entity'),
        'consumption_l1': opts.get('ev_solar_charge_consumption_phase_l1_entity'),
        'consumption_l2': opts.get('ev_solar_charge_consumption_phase_l2_entity'),
        'consumption_l3': opts.get('ev_solar_charge_consumption_phase_l3_entity'),
        'phase_switch_threshold_power': opts.get('phase_switch_threshold_power', 4000.0),
        'minimum_ev_charging_power': opts.get('ev_solar_charge_minimum_charging_power', 1380.0),
    }


def is_solar_charge_enabled() -> bool:
    """Return True if at least one production phase entity is configured."""
    opts = CONFIG.get('options', {})
    return any(
        opts.get(k)
        for k in (
            'ev_solar_charge_production_phase_l1_entity',
            'ev_solar_charge_production_phase_l2_entity',
            'ev_solar_charge_production_phase_l3_entity',
        )
    )
...
class EvSolarChargeController:
    ...
    4. If the surplus is below ``ev_solar_charge_minimum_charging_power`` nothing is done.
    ...
    All configuration is read from ``CONFIG['options']`` (config.json).
    """
    ...
    async def run_all(self, ev_devices):
        """Run the controller for every EV device.
